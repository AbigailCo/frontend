...
const CatalogoServicios = () => {
  ...
  const mostrarServicios = Array.isArray(filtradas) ? filtradas : servicios ?? [];
    ...
    <C.Contenedor titulo="Catálogo de Servicios" linkBack="-1">
      ...
      {filtradas !== null && (
        <div className="flex justify-between items-center my-4">
          <h2 className="text-sm font-semibold text-blue-800">Resultados de búsqueda</h2>
          <button onClick={handleResetFiltro} className="text-sm text-gray-700 border p-1 rounded hover:bg-gray-100">
            Ver todos
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 mt-2">
        {mostrarServicios.map(({ servicio }) => (
          <div key={servicio.id} className="bg-white rounded-xl shadow-md hover:shadow-lg transition p-4">
            <h2 className="text-md font-semibold text-gray-800">{servicio.nombre}</h2>
            <p className="text-sm text-gray-500">{servicio.descripcion}</p>
            <p className="text-violet-700 font-bold text-lg">${servicio.precio ?? "N/A"}</p>
            <p className="text-sm text-gray-600">Stock: {servicio.stock ?? "0"}</p>
            {servicio.fecha_vencimiento && (
              <p className="text-xs text-gray-400">Vence: {servicio.fecha_vencimiento}</p>
            )}
            <Link onClick={() => handleOpenModal(servicio)} className="inline-block text-sm text-violet-600 hover:underline mt-2">
              Ver más detalles
            </Link>
          </div>
        ))}
